...
mocha_typescript_1.suite("URL工具类(urlUtil)测试", function () {
    ...
    var testUrlWithFlags = 'http://localhost:7188/periodical?q=te&corpId=B2CCIRCLE#/path1/path2';
    ...
    mocha_typescript_1.suite("5.URL Path 字段测试", function () {
        it("PATH路径Item测试", function () {
            chai_1.expect(urlUtil("https://unixtime.51240.com/?p=1&q=3").getUrlPathItem(0)).eq("");
            chai_1.expect(urlUtil("https://unixtime.51240.com/p1?p=1&q=3").getUrlPathItem(0)).eq("");
            chai_1.expect(urlUtil("https://unixtime.51240.com/p1?p=1&q=3").getUrlPathItem(1)).eq("p1");
            chai_1.expect(urlUtil("https://unixtime.51240.com/p11/p21").getUrlPathItem(2)).eq("p21");
            chai_1.expect(urlUtil("https://unixtime.51240.com/p111/p211").getUrlPathItem(-1)).eq("p211");
            chai_1.expect(urlUtil("https://unixtime.51240.com/p1111/p2111/").getUrlPathItem(-1)).eq("");
            chai_1.expect(urlUtil("https://unixtime.51240.com/p11111/p21111/").getUrlPathItem(-2)).eq("p21111");
        });
    });
});
